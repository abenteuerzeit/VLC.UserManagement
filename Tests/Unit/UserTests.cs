...
namespace VLC.UserManagement.Tests
{
    ...
    public class UserTests
    {
        ...
        [Test]
        public void UpdateUserEmail_WithValidEmail_ShouldReturnNewEmail()
        {
            // Arrange
            Email email = new("test@test.com");
            Password password = new("TestPassword1");
            User newUser = new(email, password);
            Email newEmail = new("newemail@test.com");

            // Act
            newUser.ChangeEmail(newEmail);
            _userRepositoryMock.Setup(x => x.Update(newUser)).Returns(newUser);

            // Assert
            Assert.AreEqual(newEmail, newUser.Email);
        }
